...
const { routineCli, routineLogger } = require('../logger');
...
function parse_raw_task(raw_task) {
  ...
  if (dot_idx < 0) {
    routineLogger.warn(`raw_task=${raw_task}, 里面没'.'啊, 看仔细了`);
    return null;
  }
  ...
  if (task_name.length.length === 0 || choice_name.length === 0) {
    routineLogger.warn(`raw_task=${raw_task}, 命令错了, 检查一下, 点的左边是task名称, 右边是选项`);
    return null;
  }
  const is_single = list_batch_task_choice(task_name).single;
  const task = load_batch()[task_name];
  if (task === undefined) {
    routineLogger.warn(`没获取到task, 你看看${task_name}这个名称对不对`);
    return null;
  }
...
function exec_raw_batch_task(raw_task) {
  ...
  while (choice !== undefined) {
    routineCli.info(`--> ${chalk.magenta(choice.cmd)}`);
    const res = shell.exec(choice.cmd);
    if (res.stderr.trim() !== '') {
      routineCli.info(`错误: ${res.stderr}\n任务终止.`);
      shell.exit();
      return;
    }
  ...
  routineCli.info(chalk.green('done!'));
}

function exec_batch_task(task_name, choice_name) {
  routineCli.info(chalk.blue(`开始执行 ${task_name}-${choice_name}.`));
...
async function manual_connect(db) {
  ...
  switch (dia) {
    case 'postgres':
      cmd = psql_cmd(db.host, db.port, db.user, db.password, db.database);
      break;
    default:
      break;
  }
  ...
  routineCli.info(`🙈 复制粘贴: ` + chalk.black(cmd));
    ...
    await inquirer
      ...
      .then(async ans => {
          ...
          await client
            ...
            .then(res => {
              res.fields.forEach(f => {
                routineCli.info(`name: ` + chalk.green(`${f.name}`) + `, format: ` + chalk.green(`${f.format}`));
              });
            })
            .catch(err => {
              routineLogger.warn(chalk.red(`内部错误: ${err}`));
            });
          return;
        }
        await client.query(sql)
          .then(res => {
            res.rows.forEach(r => {
              routineCli.info(JSON.stringify(r));
            });
          })
          .catch(err => {
            routineCli.info(chalk.red(`错误: ${err.message}`));
          });
      });
  }
  shell.echo('bye~👋');
