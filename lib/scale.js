...
function parse_file_size_text(txt) {
  if (txt === undefined) { return -2; }
  const last_num_idx = /[0-9][\s]*?[a-zA-Z]/.exec(txt);
  if (last_num_idx === null || last_num_idx === undefined) { return -1; }
  let num = Number(txt.slice(0, last_num_idx.index + 1));
  const scale = txt.slice(last_num_idx.index + 1).toLowerCase().trim();
 ...
 * @returns {string}
 */
function get_proper_size_str(byte, option = {}) {
  let n = byte;
  let suffix = 'b';

  if (byte < one_kilo_byte) {
    //
  } else if (byte < one_mega_byte) {
    n = byte_to_byte('byte', 'kilo', byte);
    suffix = 'k';
  } else if (byte < one_giga_byte) {
    n = byte_to_byte('byte', 'mega', byte);
    suffix = 'm';
  } else if (byte < one_tera_byte) {
    n = byte_to_byte('byte', 'giga', byte);
    suffix = 'g';
  } else {
    n = byte_to_byte('byte', 'tera', byte);
    suffix = 't';
  }

  if (option.fix === undefined) {
    option.fix = 2;
  }

  return `${n.toFixed(option.fix)}${suffix}`;
}
...
module.exports = {
  ...
  get_proper_size_str: get_proper_size_str,
  Clock: Clock,
};
