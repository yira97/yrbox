...
async function change_video_codec(file, option) {
  ...
  let out_put_name;
  if (option.to_mp4_container) {
    out_put_name = get_safe_name(to_mp4_suffix(option.out));
  } else {
    out_put_name = get_safe_name(option.out);
  }
  ...
  const recode_process = new Promise((res) => {
    ...
    child.stdout.on('error', (err) => {
      mediaCli.info(chalk.yellow(`ERROR!!!!!!!!${err}`))
    });
  });
