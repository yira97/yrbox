...
async function change_video_codec(file, option) {
  ...
  const total_seconds = Math.floor(Number(get_video_stat(file).duration));
      ...
      .filter(f => {
        ...
        try {
          ...
          if (err.name === e.E.NOT_EXIST.FFPROBE_NOT_EXIST) {
            ...
            log(chalk.red(`检查文件失败.跳过文件: ${chalk.underline(f)}`));
          } else {
            log(chalk.red(`${err.name}错误.跳过文件: ${chalk.underline(f)}`));
          }
          stat = undefined;
        }
