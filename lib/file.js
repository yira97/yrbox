...
const { fileCmdLogger } = require('../lib/logger');
...
/**
 * 批量解密文件名
 * @param {string[]} encrypted_filename_list 加密的文件名列表
 * @param {string} key
 * @param {string} iv
 * @returns {Promise<string[]>}
 */
async function get_decrypt_filename(encrypted_filename_list, key, iv) {
  ...
  for (let i = 0; i < split_name.length; i++) {
    new_names.push(path.join(split_name[i].dir, decrypted_names[i]));
  }
  return new_names;
}
 ...
 */
async function encrypt_directory_filename(root, recursive, key) {
  const get_file = recursive ? find_all_file_recursive : find_all_file;
  const files = await get_file(root);
  if (!key || key.length === 0) {
    fileCmdLogger.info(`encrypt_directory_filename: invalid key: ${key}`);
    return undefined;
  }
  const { file: encrypt_filename, iv } = await get_encrypt_filename(files, key);
  for (let i = 0; i < files.length; i++) {
    await rename_file(files[i], path.basename(encrypt_filename[i]));
  }
  const pair = [];
  for (let i = 0; i < files; i++) {
    pair.push({
      old: files[i],
      new: encrypt_filename[i],
    });
  }
  return {
    file: pair,
 ...
 * @param {string} iv hex
 */
async function decrypt_directory_filename(root, recursive, key, iv) {
  //
}
